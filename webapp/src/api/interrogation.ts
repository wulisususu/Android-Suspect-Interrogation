...
export async function fetchMessages(caseId: string) {
  const { data } = await http.get(`/api/cases/${encodeURIComponent(caseId)}/messages`, {
    params: { limit: 1000 },
  })
...
export async function persistQuestionOrAnswer(caseId: string, text: string, from: '民警' | '嫌疑人') {
  const { data } = await http.post(`/work/case/${encodeURIComponent(caseId)}/message`, {
    profile: { text, from },
  })
  return unwrap<TranscriptMessage>(data)
}
...
export async function updateTranscriptMessage(caseId: string, messageId: string, text: string) {
  const { data } = await http.put(
    `/api/cases/${encodeURIComponent(caseId)}/messages/${encodeURIComponent(messageId)}`,
    { text, reason: '警官在审讯工作台修订' },
  )
  return unwrap<TranscriptMessage>(data)
}
...
export async function markTranscriptMessage(caseId: string, messageId: string, mark: RecordMark) {
  const { data } = await http.post(
    `/api/cases/${encodeURIComponent(caseId)}/messages/${encodeURIComponent(messageId)}/mark`,
    { mark },
  )
  return unwrap<TranscriptMessage>(data)
}
...
export async function streamInquiry(
  ...
  const endpoint = new URL(
    `/work/case/${encodeURIComponent(caseId)}/session/message/inquiry`,
