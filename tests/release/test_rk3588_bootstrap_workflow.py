...
def test_rk3588_service_bootstrap_workflow_installs_deploys_and_verifies_runtime():
    ...
    assert 'SOURCE_ROOT="$(find "$SOURCE_STAGE" -mindepth 1 -maxdepth 1 -type d -print -quit)"' in workflow
    assert 'test -f "$SOURCE_ROOT/deploy/control.sh"' in workflow
    assert "git sparse-checkout init --cone" not in workflow
    assert 'git -c protocol.version=2 fetch' not in workflow
    ...
    assert "SUSPECT_KIOSK_READY_URL=https://127.0.0.1:${SUSPECT_DEPLOY_PORT}/health/ready" in workflow
    assert "SUSPECT_KIOSK_URL=https://127.0.0.1:${SUSPECT_DEPLOY_PORT}/" in workflow
    assert "sudo -n bash deploy/control.sh install" in workflow
    assert "SUSPECT_HEALTH_BASE_URL=\"https://127.0.0.1:${SUSPECT_DEPLOY_PORT}\"" in workflow
    ...
    assert "curl --cacert /etc/suspect-interrogation/tls/ca.crt -fsS \"https://127.0.0.1:${SUSPECT_DEPLOY_PORT}/health/ready\"" in workflow
    assert "curl --cacert /etc/suspect-interrogation/tls/ca.crt -fsS \"https://127.0.0.1:${SUSPECT_DEPLOY_PORT}/\"" in workflow
