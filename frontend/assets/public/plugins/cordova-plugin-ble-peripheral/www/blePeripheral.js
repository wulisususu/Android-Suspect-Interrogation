cordova.define("cordova-plugin-ble-peripheral.blePeripheral", function(require, exports, module) { 
// (c) 2016 Don Coleman
//
...
var onServiceStateChangeCallback;
...
function registerServiceStateChangeCallback() {

    var serviceStateChanged = function(state) {
      console.log('serviceStateChanged', state);

      if (onServiceStateChangeCallback && typeof onServiceStateChangeCallback === 'function') {
        onServiceStateChangeCallback(state);
      }
    };

    var failure = function() {
        // this should never happen
        console.log("Failed to add serviceStateChangedListener");
    };

    cordova.exec(serviceStateChanged, failure, 'BLEPeripheral', 'setServiceStateChangedListener', []);
}
registerServiceStateChangeCallback();
...
module.exports = {
    ...
    getServiceState: function() {
        ...
        return new Promise(function(resolve, reject) {
            cordova.exec(resolve, reject, 'BLEPeripheral', 'getServiceState', []);
        });
    ...
    onServiceStateChange: function(callback) {
        onServiceStateChangeCallback = callback;
    }
