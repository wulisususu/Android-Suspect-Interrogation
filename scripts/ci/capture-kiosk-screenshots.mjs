...
try {
  ...
  await waitForSelector(client, '.workbench-mode')
  await sleep(700)
  ...
  if (!String(bodyText).includes('问答工作区') || !String(bodyText).includes('实时语音识别')) {
    throw new Error('Interrogation workbench smoke assertion failed')
  }
} finally {
  client?.close()
