...
from moss_worker import windowing
...
@pytest.mark.parametrize('allowed', [10, 8])
...
def test_hour_boundary_keeps_overlap_into_next_logical_chunk():
    windows = plan_windows(120 * MINUTE, counter(10))
    assert [(w.start_ms // MINUTE, w.end_ms // MINUTE, w.logical_chunk_index)
            for w in windows[:7]] == [
        (0, 10, 0), (8, 18, 0), (16, 26, 0), (24, 34, 0),
        (32, 42, 0), (40, 50, 0), (48, 58, 0),
    ]
...
def test_actual_interval_count_controls_each_selection():
    ...
    def exact(start, end):
        ...
        return 10753 if start == 0 and end == 10 * MINUTE else 10752
    windows = plan_windows(30 * MINUTE, exact)
    assert windows[0].end_ms == 8 * MINUTE
    assert windows[1].end_ms - windows[1].start_ms == 10 * MINUTE
    assert (0, 10 * MINUTE) in seen and (0, 8 * MINUTE) in seen
...
def test_no_estimate_or_subminimum_fallback_when_eight_does_not_fit():
    ...
    assert seen == [10 * MINUTE, 8 * MINUTE]
    with pytest.raises(ContextBudgetExceeded):
        plan_windows(60 * MINUTE, counter(10), ContextBudget(max_context_len=4096))


def test_actual_short_recording_tail_is_allowed_once():
    windows = plan_windows(13 * MINUTE, counter(10))
    assert [(w.start_ms, w.end_ms) for w in windows] == [(0, 10 * MINUTE), (8 * MINUTE, 13 * MINUTE)]
    assert len(plan_windows(MINUTE, counter(10))) == 1
    assert plan_windows(0, counter(10)) == []
...
def test_clipped_tail_retry_preserves_coverage_and_exhausts_at_eight_tier():
    failed = WindowSpec(10 * MINUTE, 13 * MINUTE, 0, 10)
    retries = plan_retry_windows(failed, counter(8))
    assert len(retries) == 1
    assert_coverage(retries, 10 * MINUTE, 13 * MINUTE)
    assert retries[0].window_minutes == 8
    with pytest.raises(RetryExhausted):
        plan_retry_windows(retries[0], counter(8))


def test_minimum_tier_is_terminal_and_no_smaller_tier_exists():
    # User-approved 2026-09-09: fallback_window == minimum_window == 8 minutes.
    # An 8-minute failure raises RetryExhausted; there is deliberately no
    # candidate tier below the minimum, and adding smaller tiers (6m/4m/...)
    # is forbidden.
    assert windowing.WINDOW_LADDER_MINUTES == (10, 8)
    assert [m for m in windowing.WINDOW_LADDER_MINUTES[1:] if m < 8] == []
    failed = WindowSpec(0, 8 * MINUTE, 0, 8)
    with pytest.raises(RetryExhausted):
        plan_retry_windows(failed, counter(8))
