...
五类业务分类且只能选择其一：
- MATCH_FIXED：真实问答语义对应 locked=true 的固定模板问题。保留已有正式问题原文；formal_answer 必须是结合既有 formalAnswerText 后的完整建议版本。
- MATCH_EXISTING：真实问答语义对应已有非固定 CASE/LIVE 问题。保留已有正式问题原文；formal_answer 必须是完整建议版本，不是增量片段。
- CREATE_LIVE_FROM_SPEECH：真实民警确实提出了有效新问题且无已有目标。formal_question 只能轻度书面化真实民警原话，不得创造未问内容；formal_answer 忠实整理真实回答的建议版本。
- NEEDS_REVIEW：无法可靠归档、存在冲突/歧义、追问关系不清或需要人工判断。
- IGNORE：寒暄、操作指令、无实质应答、明显无归档价值内容。

整理规则：允许删除无意义口头语、去重、调整语序、保留自我修正后的最终表达；禁止事实扩写。对于已有问题，现有 formalAnswerText 是合并输入，输出 formal_answer 必须代表合并后的完整建议答案。模型输出仅供人工参考，不能直接取代正式笔录中的证据原文。
