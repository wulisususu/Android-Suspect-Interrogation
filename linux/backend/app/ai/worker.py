...
def worker_main(conn: Connection, *, kind: str, mode: str, spec: ModelSpec, model_root: str) -> None:
    ...
    try:
        ...
        while True:
            ...
            try:
                if op == "generate":
                    result = engine.generate(payload["prompt"], session_id=payload["session_id"], options=payload.get("options"))
                    conn.send({"type": "result", "data": result})
                elif op == "stream_llm":
                    for chunk in engine.stream(payload["prompt"], session_id=payload["session_id"], options=payload.get("options")):
                        conn.send({"type": "chunk", "data": chunk})
                    conn.send({"type": "done"})
                elif op == "transcribe":
                    result = engine.transcribe(payload["audio"], session_id=payload["session_id"], options=payload.get("options"))
                    conn.send({"type": "result", "data": result})
                elif op == "stream_asr":
                    for chunk in engine.stream(payload["audio"], session_id=payload["session_id"], options=payload.get("options")):
                        conn.send({"type": "chunk", "data": chunk})
                    conn.send({"type": "done"})
                elif op == "ocr":
                    result = engine.recognize(payload["image"], capability=payload.get("capability", "text"), session_id=payload["session_id"], options=payload.get("options"))
                    conn.send({"type": "result", "data": result})
                else:
                ...
                conn.send({"type": "error", "code": "WORKER_ERROR", "message": str(exc), "details": {"traceback": traceback.format_exc(limit=8)}})
    except (EOFError, BrokenPipeError, KeyboardInterrupt):
        return
    except Exception as exc:
        try:
            conn.send({"type": "startup_error" if engine is None else "error", "code": exc.code if isinstance(exc, AIError) else "WORKER_STARTUP_ERROR", "message": str(exc), "details": getattr(exc, "details", {})})
        except Exception:
            pass
    finally:
        try:
