...
def ai_health(request: Request):
    return _supervisor(request).health()


@router.get("/capabilities")
def ai_capabilities(request: Request):
    return _supervisor(request).capabilities()
...
def llm_stream(body: LLMGenerateRequest, request: Request):
    ...
    def generate():
        try:
            for chunk in supervisor.stream_llm(body.prompt, session_id=body.session_id, options=body.options):
                yield json.dumps(asdict(chunk), ensure_ascii=False) + "\n"
        except AIError as exc:
            yield json.dumps(
...
def llm_cancel(request: Request):
    _supervisor(request).cancel("llm")
...
async def asr_transcribe(body: BinaryInferenceRequest, request: Request):
    try:
        audio = base64.b64decode(body.data_b64, validate=True)
    except ValueError as exc:
        raise HTTPException(status_code=422, detail={"code": "INVALID_BASE64"}) from exc
    try:
        return asdict(
            await asyncio.to_thread(
                _supervisor(request).transcribe,
                audio,
                session_id=body.session_id,
                options=body.options,
            )
...
async def asr_stream(websocket: WebSocket):
    ...
    supervisor = websocket.app.state.ai_supervisor
            ...
            if chunk is not None:
                ...
                try:
                    ...
                    if partials:
                        ...
                        payload["final"] = False
                        await websocket.send_json({"type": "partial", "result": payload})
                except AIError as exc:
                ...
                if command.get("type") == "cancel":
                    ...
                    try:
                        ...
                        await websocket.send_json({"type": "final", "result": asdict(result)})
                    except AIError as exc:
                        await websocket.send_json({"type": "error", "error": {"code": exc.code, "message": exc.message}})
                    audio.clear()
...
async def ocr_recognize(body: OCRRequest, request: Request):
    try:
        image = base64.b64decode(body.data_b64, validate=True)
    except ValueError as exc:
        raise HTTPException(status_code=422, detail={"code": "INVALID_BASE64"}) from exc
    try:
        return asdict(
            await asyncio.to_thread(
                _supervisor(request).recognize,
                image,
