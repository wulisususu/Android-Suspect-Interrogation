...
`MossTranscriptionCoordinator` is owned by the FastAPI lifespan (started with the loop, stopped on shutdown, mirroring `QARoutingCoordinator`). One asyncio task wakes on a fixed interval and, per transcription row in a working state, runs the synchronous socket client through `asyncio.to_thread`: `get_job` → persist state/windows snapshot; on `COMPLETED` → `get_result` → append a revision; on `RECOVERY_REQUIRED`/`FAILED`/`CANCELLED` → persist the terminal state and stop polling that row. Poll failures (worker down) leave the persisted row untouched and retry on a later tick.

Incremental transcript (user requirement ⑤, 2026-09-09): `get_job` windows carry a `segments` key for `DONE` windows only, serialized by the same `NormalizedSegment.to_dict` used by `get_result`. While the row is in an active poll state, whenever the count of published DONE-window segments grows beyond the latest revision, the coordinator appends an incremental revision (`revision_no` max+1, provenance marked `partial: true`, current mapping snapshot); terminal states other than `COMPLETED` never write a transcript revision. `COMPLETED` still writes exactly one final revision (merged/deduplicated). The status payload exposes `revisionNo` (null until the first revision); a transcript revision is always the full known-segment snapshot, so clients replace (never append) on `revisionNo` change. `role` in the transcript is the real-time mapping display value and may change without `revisionNo` moving.
