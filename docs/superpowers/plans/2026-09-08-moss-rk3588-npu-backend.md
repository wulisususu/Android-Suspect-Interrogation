...
Board callback evidence (2026-09-08, same private 1.3.0 runtime and 16K model): the 666-token golden prefill produced 259 token callbacks ending in EOS `151645`, then FINISH; `perf.generate_tokens` was 258. A diagnostic-only 16-token generation cap produced 16 token callbacks, no EOS, and still FINISH with `perf.generate_tokens=15`. Count actual NORMAL/WAITING token events, including EOS, rather than treating the performance decode-step count or FINISH as completion proof. The production cap remains 5120. Evidence is retained outside Git under `E:/moss-build-assets/context-16k/callback-evidence.json`.

Clear KV cache explicitly before every decode: the board reused a 15-token prompt prefix after startup self-test despite `keep_history=0`, reporting 651 newly prefetched tokens for a 666-token input. The first child correctly failed closed; `rkllm_clear_kv_cache(handle, 0, nullptr, nullptr)` restored independent full-prefill accounting. The complete 45-second engineering WAV then yielded 7 segments, S01/S02, text similarity 1.0 and maximum timestamp difference 20 ms versus PyTorch. This is not long-window or real-recording acceptance; evidence is `E:/moss-build-assets/runtime-child-v2-evidence.json`.
