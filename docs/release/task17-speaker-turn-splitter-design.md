...
### 生产路径的现实约束（worker 没有声纹参考）

`speech_worker` **不得接触声纹库**（biometric reference 进 worker 会让耦合失控），因此生产路径里 `reference is None`：
- **阶段一无法运行**（它就是"与参考比对"）
- **规则 B 无法运行**（它需要两侧 cosRef 角色证据）
- 只剩相邻窗余弦（规则 A）

若只保留规则 A（≥2 连续 < 0.50），实测混说段只有 **1 个** 低于阈值的窗对 → 不切 → 返回 ambiguous → 后端据 `overlap` 判 UNKNOWN。**这是安全的**（绝不把民警问句记到嫌疑人名下），但只是"保守正确"。

**改进（已用同一份语料量化）**：参考无关路径增加**深度单谷**规则：
...
混说段谷值 **0.243**，同人段内最小谷值 **0.349** → 取 **0.30** 可覆盖前者且不触碰后者。

生产规则因此为：**"≥2 连续窗 < 0.50，或 单谷 < 0.30"** → 切分；切出的每个 turn 各自出 ASR + 各自 embedding，**角色判定仍由 `AsrCaptureService` 完成**（worker 只回答"换没换人"）。这样混说段被切成两段后：民警段对嫌疑人参考得分低 → UNKNOWN/民警（取决于是否绑定民警声纹），嫌疑人段 → SUSPECT，而**不再是整段 SUSPECT**。

验收器 `run_splitter_corpus_check.py --no-reference` 即模拟这一生产路径：要求"切成 ≥2 段 或 标 ambiguous"，且**绝不**整段置信单一 SUSPECT。
