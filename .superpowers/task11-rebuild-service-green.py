...
voiceprint = ROOT / "linux/backend/app/services/voiceprint_service.py"
text = voiceprint.read_text(encoding="utf-8")
anchor = '''    def _build_reference(self, pcm: bytes) -> dict[str, Any]:\n        prepared = self._prepare_reference_audio(pcm)\n        return self._build_reference_for_backend(prepared, _XVECTOR)\n'''
replacement = '''    def build_reference_for_backend(self, pcm: bytes, backend: str) -> dict[str, Any]:\n        """Build one model-specific reference from source PCM using enrollment semantics."""\n        backend_key = str(backend or "").strip().lower()\n        if backend_key not in _ENROLLMENT_BACKENDS:\n            raise DomainError(\n                "VOICEPRINT_BACKEND_UNSUPPORTED",\n                "声纹 reference 只能使用已支持的具体 embedding backend",\n                400,\n                data={"backend_key": backend_key},\n            )\n        prepared = self._prepare_reference_audio(pcm)\n        return self._build_reference_for_backend(prepared, backend_key)\n\n    def _build_reference(self, pcm: bytes) -> dict[str, Any]:\n        return self.build_reference_for_backend(pcm, _XVECTOR)\n'''
if anchor not in text:
    raise SystemExit("Task11 VoiceprintService anchor missing")
voiceprint.write_text(text.replace(anchor, replacement, 1), encoding="utf-8")

service = ROOT / "linux/backend/app/services/speaker_reference_rebuild.py"
if service.exists():
    raise SystemExit("Task11 rebuild service unexpectedly already exists")
service.write_text(r'''from __future__ import annotations
...
print("Task11 rebuild service apply complete")
